...
${matchedContext || "No direct string match found, rely on overall database summary above."}`;
    ...
    if (!groqApiKey) {
      return NextResponse.json({ 
        reply: "⚠️ Groq API Key is missing in server environment variables." 
      });
    }
    ...
    // Multi-Model Fallback Array to prevent model name mismatch errors
    const modelsToTry = [
    ...
    for (const model of modelsToTry) {
      try {
        ...
        console.warn(`Model ${model} failed, attempting next model in fallback array...`);
      }
    }

    // Local Fallback if all API models fail
    return NextResponse.json({ 
