...
export default function ChatbotPage() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    const newUserMsg: Message = { id: Date.now(), role: 'user', content: userMessageText }
    
    // Update messages state including new user message
    ...
    try {
      // Calling the backend API route we created earlier
      ...
      const errorMsg: Message = {
        id: Date.now() + 1,
        role: 'ai',
        content: 'Network or server error connecting to PAIMANA backend database.'
      }
          ...
          {isLoading && (
            <div className="flex gap-4 justify-start">
              ...
              <div className="bg-white border border-slate-200 p-4 rounded-2xl rounded-bl-sm flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-slate-500">AI is analyzing database...</span>
              </div>
            </div>
          )}
