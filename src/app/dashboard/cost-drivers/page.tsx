...
import { projectService, riskService } from '../../services/api';
...
export const CostDrivers: React.FC = () => {
  ...
  const [, setLoading] = useState<boolean>(true); // Using array destructuring to bypass unused variable check if needed, or just keep it if used elsewhere.

  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        const allProj: Project[] = await projectService.getAll();
        setProjects(allProj);

        const currentProj =
          allProj.find((p) => p.id === selectedProjectId) || allProj[0] || null;

        setProject(currentProj);

        if (currentProj) {
          const riskDrivers: RiskDriver[] = await riskService.getDrivers(currentProj.id);
          setDrivers(riskDrivers);
        } else {
          setDrivers([]);
        }
      } catch (err) {
    ...
    >
      ...
      >
        <div>
          ...
          <div style={{ fontSize: '0.82rem', color: '#718096' }}>
            Deconstructing predictive risk factors using SHAP (SHapley Additive
            exPlanations) values from MoSPI CUF parameters
          </div>
        </div>

        {/* Project Selector Dropdown */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
          <label style={{ fontSize: '0.82rem', fontWeight: 700, color: '#17365D' }}>
            Select Project:
          </label>
          <select
      ...
      {project && (
        <>
                ...
                <div style={{ textAlign: 'center' }}>
                  ...
                  <div
                    style={{
                      fontSize: '2rem',
                      fontWeight: 900,
                      color: project.riskScore >= 70 ? '#FEB2B2' : '#FBD38D',
                    }}
                  >
                    {project.riskScore}{' '}
            ...
            >
              <div>
                <h3 style={{ fontSize: '1.2rem', color: '#17365D' }}>
                  Risk Factor Impact Breakdown (SHAP Feature Attribution)
                </h3>
                <div style={{ fontSize: '0.8rem', color: '#718096' }}>
                  Relative contribution of top bottleneck parameters driving risk
                  score to {project.riskScore}/100
                </div>
              </div>
          ...
          <div
            className="card-paimana"
            style={{ borderLeft: '5px solid #F59A00' }}
          >
            <div
              style={{
                display: 'flex',
            ...
            >
              ...
              <h3 style={{ fontSize: '1.1rem', color: '#17365D' }}>
                PAIMANA AI Recommended Strategic Interventions
              </h3>
            </div>
            <div
              style={{
            ...
            >
              ...
              >
                ...
                >
                  1. Fast-Track Inter-Departmental Clearances
                </div>
                <div style={{ color: '#718096' }}>
                  Issue urgent directive to SLAO & Ministry of Environment for
                  fast-tracked stage-2 forest clearance.
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                >
                  2. Accelerated Equity Release
                </div>
                <div style={{ color: '#718096' }}>
                  Sanction Q3 grant release to prevent contractor liquidity
                  bottlenecks and interest penalty accumulation.
                </div>
              </div>
            </div>
          </div>
