...
export default function ProjectsPage() {
  ...
  const handleExportCsv = (): void => {
    ...
    const rows = filteredProjects.map((p) => [
      `"${p.id}"`,
      `"${getProjectName(p).replace(/"/g, '""')}"`,
      `"${getProjectSector(p).replace(/"/g, '""')}"`,
      `"${getProjectState(p).replace(/"/g, '""')}"`,
      getOriginalCost(p),
      getRevisedCost(p),
      getCostOverrun(p),
      `"${(p.contractor || 'N/A').replace(/"/g, '""')}"`,
    ]);
    ...
    link.setAttribute(
      'download',
      searchTerm
        ? `PAIMANA_Projects_${searchTerm.replace(/[^a-zA-Z0-9]/g, '_')}.csv`
        : 'PAIMANA_MoSPI_Projects_Registry.csv'
    );
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <button
              ...
              className="px-4 py-2.5 bg-white border border-slate-200 hover:bg-slate-50 text-[#17365D] text-xs font-bold rounded-xl shadow-xs transition-all flex items-center gap-2 cursor-pointer active:scale-95"
            >
              <Download className="w-4 h-4 text-[#F59A00]" />
