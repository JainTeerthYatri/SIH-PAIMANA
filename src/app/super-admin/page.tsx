...
import { 
  ShieldAlert, Users, Activity, Key, LogOut, 
  ShieldCheck, Database, RefreshCcw, Plus, X, Lock, Copy, Check
} from 'lucide-react'
...
interface Officer {
  ...
  user_metadata: { 
    role?: string
    monthly_admin_code?: string | null
  }
}
  ...
  const handleRegenerateCode = async (userId: string) => {
    ...
    try {
      ...
      setOfficers(prev => prev.map(o => 
        o.id === userId 
          ? { ...o, user_metadata: { ...o.user_metadata, monthly_admin_code: data.newCode } }
          : o
      ))
    } catch (err: any) {
      alert(err.message)
            ...
            <table className="min-w-full divide-y divide-slate-800">
              <thead className="bg-slate-950/50">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">8-Digit Monthly Code</th>
              ...
              <tbody className="divide-y divide-slate-800">
                {officers.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-10 text-center text-slate-500">Fetching network identities...</td>
                  </tr>
                ) : (
                  officers.map((officer) => (
                    <tr key={officer.id} className="hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {officer.user_metadata?.role === 'admin' ? (
                          officer.user_metadata?.monthly_admin_code ? (
                            <div className="flex items-center gap-2">
                              <code className="font-mono text-amber-400 font-bold tracking-widest text-sm bg-amber-500/10 px-2.5 py-1 rounded border border-amber-500/20">
                                {officer.user_metadata.monthly_admin_code}
                              </code>
                              <button
                                onClick={() => handleCopyCode(officer.user_metadata.monthly_admin_code!, officer.id)}
                                className="p-1.5 rounded-lg hover:bg-slate-700 text-slate-400 hover:text-white transition-colors"
                                title="Copy Code"
                              >
                                {copiedId === officer.id ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                              </button>
                            </div>
                          ) : (
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {officer.user_metadata?.role === 'admin' && (
                          <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/30 rounded-lg text-xs font-bold transition-all disabled:opacity-50"
                          >
                            {regeneratingId === officer.id ? (
                              <Activity className="w-3.5 h-3.5 animate-spin" />
                            ) : (
                              <RefreshCcw className="w-3.5 h-3.5" />
                            )}
                            {regeneratingId === officer.id ? 'Generating...' : 'Regenerate'}
                          </button>
                        )}
                      </td>
                    </tr>
