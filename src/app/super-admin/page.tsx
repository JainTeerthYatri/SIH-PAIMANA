...
export default function SuperAdminPage() {
  ...
  // 🔄 Regenerate 8-Digit Monthly Code
  ...
  const handleDeleteUser = async (userId: string, email: string) => {
    if (!confirm(`⚠️ ARE YOU SURE?\n\nThis will permanently delete ${email} from Supabase Auth!`)) {
      return
    }
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewUser({ email: '', password: '', role: 'admin', monthlyCode: '' })
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-10 space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl flex items-center gap-4 shadow-xl relative overflow-hidden">
            ...
            <div className="p-4 bg-purple-500/10 text-purple-500 rounded-xl relative z-10">
          ...
          <div className="p-4 sm:p-6 border-b border-slate-800 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div>
              ...
              <p className="text-xs sm:text-sm text-slate-500 mt-1">
                Manage network access, roles, ciphers, and account deletions
              </p>
            </div>
                    ...
                    <tr key={officer.id} className="hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        {officer.user_metadata?.role === 'super_admin' ? (
                          <span className="text-red-400 font-bold uppercase tracking-wider text-[10px] sm:text-xs bg-red-500/10 border border-red-500/20 px-2 py-1 rounded">
                            Super Admin
                          </span>
                        ) : officer.user_metadata?.role === 'admin' ? (
                          <span className="text-emerald-400 font-bold uppercase tracking-wider text-[10px] sm:text-xs bg-emerald-500/10 border border-emerald-500/20 px-2 py-1 rounded">
                            Admin
                          </span>
                        ) : (
                          <span className="text-blue-400 font-bold uppercase tracking-wider text-[10px] sm:text-xs bg-blue-500/10 border border-blue-500/20 px-2 py-1 rounded">
                            Officer
                          </span>
                        )}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        {officer.user_metadata?.role === 'admin' ? (
                          <div className="flex items-center gap-3">
                            ...
                            >
                              <RefreshCcw
                                className={`w-3 h-3 ${actionLoadingId === officer.id ? 'animate-spin' : ''}`}
                              />
                              Regenerate
                            </button>
                          </div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-xs sm:text-sm text-slate-400">
                        {formatDate(officer.created_at)}
                      </td>

                      <td className="px-6 py-4 whitespace-nowrap text-xs sm:text-sm text-slate-500 flex items-center gap-2">
                        {officer.user_metadata?.role !== 'super_admin' ? (
                          <>
                            <button
                              onClick={() => handleResetPassword(officer.id, officer.email)}
                              className="bg-amber-500/10 hover:bg-amber-500/20 text-xs text-amber-400 px-2.5 py-1.5 rounded font-semibold transition-all border border-amber-500/30"
                            >
                              🔑 Reset Pass
                            </button>
                            <button
                              onClick={() => handleDeleteUser(officer.id, officer.email)}
                              disabled={actionLoadingId === officer.id}
                              className="bg-red-500/10 hover:bg-red-600 hover:text-white text-xs text-red-400 px-2.5 py-1.5 rounded font-semibold transition-all border border-red-500/30 flex items-center gap-1 disabled:opacity-50"
                            >
                              <Trash2 className="w-3.5 h-3.5" /> Delete
                            </button>
                          </>
                        ) : (
                          <span className="text-slate-600 text-xs italic">Protected Root</span>
                        )}
                      </td>
                    </tr>
                  ))
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md shadow-2xl relative">
            ...
            <form onSubmit={handleCreateUser} className="space-y-3.5">
              ...
              <button
                ...
                className="w-full mt-4 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2.5 rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-xs shadow-lg"
              >
                {modalStatus === 'loading' ? (
